...
void Player::Update() {
    ...
    if (InputManager::Instance()->IsKeyPress(SDLK_UP) || InputManager::Instance()->IsKeyPress(SDLK_w)) {
        pos_accum_.y -= 1;
        input_this_frame_ = true;
    }
    if (InputManager::Instance()->IsKeyPress(SDLK_LEFT) || InputManager::Instance()->IsKeyPress(SDLK_a)) {
        pos_accum_.x -= 1;
        input_this_frame_ = true;
    }
    if (InputManager::Instance()->IsKeyPress(SDLK_RIGHT) || InputManager::Instance()->IsKeyPress(SDLK_d)) {
        pos_accum_.x += 1;
        input_this_frame_ = true;
    }
...
void Player::UpdatePosition() {
    ...
    if (input_this_frame_) {
        ...
        current_angle_deg_ = current_angle_ * 180 / M_PI;
...
void Player::Render() {
    ...
    for (int i = 0; i < current_length_; i++) {
        TextureManger::Instance()->DrawFrame(texture_id,
                                             tail_[i].x,
                                             tail_[i].y,
                                             width,
                                             height,
...
Vector2 Player::NewFruitLocation(int width_max, int height_max) {
    ...
    if (current_length_ > 1) {
        ...
        for (int i = 0; i < number_pos_needed; i++) {
            ...
            int dist = temp_vect.DistanceSquared(tail_positions_[i]);
            if (dist < size_squared) {
...
int Player::GetRandomValue(int i, int max) {
    ...
    std::uniform_int_distribution<> distr(i, max - 1);// Define the range
