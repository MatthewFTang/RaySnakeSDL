...
void TextureManger::DrawImage(const std::string &id) {
  ...
  dest_rect.w = w;
  dest_rect.h = h;
  SDL_RenderCopyEx(Application::Instance()->GetRenderer(), texture_map_[id],
                   &src_rect, &dest_rect, 0, nullptr,
                   SDL_RendererFlip::SDL_FLIP_NONE);
}

void TextureManger::DrawImageWithSize(const std::string &id, int x_location,
                                      int y_location, int height, int width) {
  Vector2 text_size = GetTextureDimensions(id);
  SDL_Rect src_rect;
  SDL_Rect dest_rect;
  src_rect.x = src_rect.y = 0;
  ...
  dest_rect.h = width;
  SDL_RenderCopyEx(Application::Instance()->GetRenderer(), texture_map_[id],
                   &src_rect, &dest_rect, 0, nullptr, SDL_FLIP_NONE);
}
...
void TextureManger::DrawFrame(const std::string &id, int x_offset, int y_offset,
                              ...
                              int current_frame, float rotation,
  ...
  SDL_RenderCopyEx(Application::Instance()->GetRenderer(), texture_map_[id],
                   &src_rect, &dest_rect, rotation, nullptr, flip);
}
...
Vector2 TextureManger::GetTextureDimensions(const std::string &id) {
  ...
  if (texture_map_.find(id) !=
    ...
    if (SDL_QueryTexture(texture, nullptr, nullptr, &text_size_w,
                         &text_size_h) ==
        0) { // Successfully queried texture size
